...
function Patrimonio() {
  ...
  const { data } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const [inv, inm, pres] = await Promise.all([
  ...
  const totals = useMemo(() => {
    const inversiones = (data?.inversiones ?? []).reduce((s: number, i: any) => s + Number(i.cantidad) * Number(i.valor_actual || i.precio_compra), 0);
